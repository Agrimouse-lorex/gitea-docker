...
test.describe('Basic requests as admin user', () => {
    ...
    test.describe('Basic requests as authenticated user', () => {
        ...
        test('Delete newly created repo', async()=>{
                let repoName = `${baseName}`
                console.log('Repository Name for Delete: ', repoName)
                const getOwnerName = await api.get(`${apiURL}/user`)
                const body = await getOwnerName.json()
                const owner = body.username
                let response = await api.delete(`${apiURL}/repos/${owner}/${repoName}`)
                if(response.status() === 404) {
                    repoName = `${baseName}-${datePref}`
                    response = await api.delete(`${apiURL}/repos/${owner}/${repoName}`)
                }
                console.log("Delete Repository status code: ", response.status())
                expect(response.status()).toBe(204);
                console.log(`Repository with name: ${repoName} was sucessfully Deleted`)
...
test.describe('Deleting external data', () => {
